...
/*   Updated: 2021/05/03 14:56:48 by flwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_sprite_v(t_all *all, double disth)
{
	int	i;
		...
		+ pow(all->cam.ry - all->fmt.ppy, 2)))
	{
		...
		while (i < all->spr.len)
		{
			if (i < all->spr.nact && all->spr.act[all->spr.nact - 1] == i)
				break ;
			else if (all->cam.rx >= all->spr.sx[i] && all->cam.rx <= \
				all->spr.sx[i] + 64 && all->cam.ry >= all->spr.sy[i]
				&& all->cam.ry <= all->spr.sy[i] + 64 )
			{
...
void	find_wall_sprite_v(t_all *all, double xo, double yo, double disth)
{
	...
	while (1)
	{
		if (all->cam.ry > (all->fmt.map_size - 1) * 64 || all->cam.rx < 0
			...
			[(int)all->cam.rx / 64] == '1')
		{
			all->cam.dist = sqrt(pow(all->cam.rx - all->fmt.ppx, 2) \
					+ pow(all->cam.ry - all->fmt.ppy, 2));
			all->cam.sidehit = 'v';
			break ;
		}
		find_sprite_v(all, disth);
		all->cam.rx += xo;
		all->cam.ry += yo;
	}
}
...
void	check_v(t_all *all, double disth)
{
	...
	if (all->cam.ra > PI / 2 && all->cam.ra < PI * 3 / 2)
	{
		...
		all->cam.ry = (all->fmt.ppx - all->cam.rx) * -tan(all->cam.ra) \
					  + all->fmt.ppy;
		xo = -64;
